...
export async function newPass(req, res) {
 ...
 if (
    !name || !roll || !sem || !where || !purpose || 
    !transport || !outtime || !date || 
    ownResponsibility === undefined || ownResponsibility === null
  ){
    return response_400(res, 'Some fields are missing! Please fill all inputs.');
  }
  let newForm = Form({
    name: name,
    roll: roll,
